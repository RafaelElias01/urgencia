 ...
 *
 * <p><b>Causa raiz:</b> {@code AnexoStorageService.removerAntigosDoTipo}
 ...
 * ja deletado — Hibernate recusa com {@code ObjectDeletedException}.</p>
 *
 * <p>Por que precisa ser {@code @SpringBootTest} (e nao
...
class RegistrarEnvioDuasVezesIntegrationTest {
    ...
    void preparar() throws Exception {
        ...
        // Documento clinico PDF valido, obrigatorio para o 1o (e o 2o) registro de envio.
    ...
    void registrarEnvioDuasVezesAposReabrirNaoQuebra() throws Exception {
        ...
        long anexosSolicitacaoAposPrimeiroEnvio = anexoRepo.findByProcessoIdAndTipo(processoId, TipoAnexo.SOLICITACAO_AVALIADOR).size();
        ...
        mvc.perform(post("/processos/" + processoId + "/reabrir").with(csrf()))
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.status().is3xxRedirection());
        assertThat(processoRepo.findById(processoId).orElseThrow().getStatus()).isEqualTo(StatusProcesso.ENVIADO);
    ...
    void substituirComprovanteSntDuasVezesMantemApenasUmAnexo() throws Exception {
        ...
        mvc.perform(multipart("/processos/" + deferidoId + "/comprovante-snt")
                        .file(new MockMultipartFile("arquivo", "comprovante1.pdf", "application/pdf", pdfValido()))
                        .with(csrf()))
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.status().is3xxRedirection());
        ...
        var resultadoSegundo = mvc.perform(multipart("/processos/" + deferidoId + "/comprovante-snt")
                        .file(new MockMultipartFile("arquivo", "comprovante2.pdf", "application/pdf", pdfValido()))
                        .with(csrf()))
                .andReturn();
    ...
    void falhaAoSubstituirComprovanteSntPreservaOAnexoOriginal() throws Exception {
        ...
        mvc.perform(multipart("/processos/" + deferidoId + "/comprovante-snt")
                        .file(new MockMultipartFile("arquivo", "comprovante-original.pdf", "application/pdf", pdfValido()))
                        .with(csrf()))
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.status().is3xxRedirection());
        ...
        var resultadoFalha = mvc.perform(multipart("/processos/" + deferidoId + "/comprovante-snt")
                        .file(new MockMultipartFile("arquivo", "malware.exe", "application/octet-stream", "x".getBytes()))
                        .with(csrf()))
                .andReturn();
